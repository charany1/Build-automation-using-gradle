...
var AUTOCO_MNT_PROP = "AUTOCO_MOUNT_POINT";
var THIRD_PARTY_MNT_PROP = "THIRD_PARTY_MOUNT_POINT";
var RESOLVE_LINKS_SEPARATOR = "::=:=::";
...
$._ext = {
    ...
    writeToFile : function( xmlData, xmlFileName ) {
	    ...
	    try {
	    	...
	    	var XMLFolder = $._ext.normalizeFolder(extractedFolderPath);
	        if(!XMLFolder.exists) {
	        	XMLFolder.create();
	        } 
	        var XMLFile;
	...
	normalizeFolder: function ( filePath ) {
		var retVal;
		do {
			try {
	...
	normalizeFile : function (filePath) {
		...
		if (retVal.fsName == filePath)
			return retVal;
		/*if (retVal.fsName.match(/^\/Volumes\//i) == null) // File path isn't a Volumes path
			return retVal;*/
		var rootVolumeName = File("/").parent.displayName;
	...
	getNormalizedLinkPath : function(filePath) {
		var retVal = File(filePath);
		if (File.fs != "Macintosh")
			return filePath;
		if (retVal.fsName == filePath)
			return filePath;
		var rootVolumeName = File("/").parent.displayName.toUpperCase();
		if(filePath.toUpperCase().indexOf(rootVolumeName) == 0)
			return (retVal.fsName.replace(/^\/Volumes\//,""));
		return retVal.fsName;
	},
	
	getLinkedFileMap : function(tmpFileName) {
		var tmpFilePath = "~/.zm/tmp/"+tmpFileName;
		var tmpFile = File(tmpFilePath);
		var fileMap = $._ext.parseFile(tmpFilePath, RESOLVE_LINKS_SEPARATOR, "resolveprops");
        if (tmpFile.exists) {
			tmpFile.remove();
		}
        return fileMap;
	...
	getZoomProps : function() {
		...
		zoomProps = $._ext.parseFile(zoomPropFilePath, "=", "zoomprops");
		//return zoomProps;
	},
	...
	parseFile : function (filePath, separator, requestor) {
		...
		if(file.exists) {
			...
			while(!file.eof) {
	            ...
	            if(fileMapStr && fileMapStr.indexOf(separator) > 0) {
	                ...
	                if(requestor == "resolveprops") {
	                	var resolveObj = {};
	                	resolveObj.newPath = strArr[1];
	                	resolveObj.remotePath = strArr[2];
	                	props[strArr[0]] = resolveObj;
	                } else {
	...
	createProgress : function(title, message, hasCancelButton) {  
		...
		writeToPropFile : function( propFileName, propStr ) {
		    ...
		    try {
		        ...
		        var propFolder = $._ext.normalizeFolder(propFolderPath);
		...
		getUserProps : function(fileName) {
			...
			userSettings = $._ext.parseFile(userPropFilePath, "=", "userprops");
		...
		searchLinksFromExtFolders : function(resolveObj) {
			if (!resolveObj || !resolveObj.remotePath)
				return null;
			var autoCoFolder = $._ext.getZoomProperty(AUTOCO_MNT_PROP);
			var thirdPartyMountArray = $._ext.getThirdPartyMountArray();
			if(!autoCoFolder && !thirdPartyMountArray)
				return null;
			var remotePath = resolveObj.remotePath;
			var separator = '\\';
			if(File.fs && File.fs.indexOf("Windows") < 0)
            	separator = '/';
			if(autoCoFolder) {
				autoCoFolder = autoCoFolder.replace('\\:', ':');
				var relinkFile = File(autoCoFolder + separator + remotePath);
				if (relinkFile && relinkFile.exists)
					return relinkFile;
			}
			if(thirdPartyMountArray) {
				for (var index = 0; index < thirdPartyMountArray.length; index++) {
					var thirdPartyMtPoint = thirdPartyMountArray[index];
					if (!thirdPartyMtPoint)
						continue;
					thirdPartyMtPoint = thirdPartyMtPoint.replace('\\:', ':');
					var relinkFile = File(thirdPartyMtPoint + separator + remotePath);
					if(relinkFile && relinkFile.exists)
						return relinkFile;
				}
			}
		...
		getThirdPartyMountArray : function () {
			var thirdPartyMountPoints = [];
			if(!zoomProps)
				$._ext.getZoomProps();
			if(zoomProps) {
				for (var prop in zoomProps) {
					if(prop.indexOf(THIRD_PARTY_MNT_PROP) == 0) {
						thirdPartyMountPoints.push(zoomProps[prop]);
					}
				}
			}
			
			return thirdPartyMountPoints;
		},
