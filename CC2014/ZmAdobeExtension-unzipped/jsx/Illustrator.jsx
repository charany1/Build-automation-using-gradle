    			...
    			if(docLength > 0) {
    	            if(app.activeDocument) {
    	    		    ...
    	    		    if(pathURI){    
    	                  ...
    	                  if ((!docPath || (docPath && docPath == ""))  && isRefresh !== 'true') {
    	                	  filePath = "";
	...
	getAILayerLinkDetails : function( thisObj, isLink ) {
	    ...
	    if( isLink ){
	        try {
	        	...
	        	var linkedPath = $._ext.getNormalizedFSName(thisObj.file.fullName);
...
//	        }
	...
	iterateAIAndRelink : function(targetLayer, index, fileMap) {
		if(fileMap && (index in fileMap)) {
	        var replacePath = fileMap[index];
	        var replaceFile = $._ext.normalizeFile(replacePath);
	        if(replaceFile.exists && typeof targetLayer.relink == "function") {
